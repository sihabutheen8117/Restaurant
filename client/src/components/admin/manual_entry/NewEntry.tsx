        ...
        <div className="m-4 text-sm">
                <table className="min-w-[250px] border border-gray-300 rounded-md overflow-hidden w-fit text-sm shadow-sm">
                    ...
                    <tr className="border-b border-gray-200 hover:bg-amber-100  transition-all duration-300">
                        <th className="text-left px-4 py-2 font-medium text-gray-600 bg-gray-100 w-32">Foods</th>
                        <td className="px-4 py-2 text-gray-900">{props.selected_food.length}</td>
                    </tr>
                    {
                          (() => {
                              const totals = props.selected_food.reduce(
                              (acc: { totalQty: number; totalCost: number }, item: any) => {
                              const qty = parseInt(item.quandity || 0);
                              const price = item.offer_price === -1 ? item.price : item.offer_price;
                              acc.totalQty += qty;
                              acc.totalCost += qty * price;
                              return acc;
                              },
                              ...
                              return (
                                <>
                                  <tr className="border-b border-gray-200 hover:bg-amber-100  transition-all duration-300">
                                      <th className="text-left px-4 py-2 font-medium text-gray-600 bg-gray-100">Quantity</th>
                                      <td className="px-4 py-2 text-gray-900">{totals.totalQty}</td>
                                  </tr>
                                  <tr className="hover:bg-amber-100 transition-all duration-300">
                                      <th className="text-left px-4 py-2 font-medium text-gray-600 bg-gray-100">Total cost</th>
                                      <td className="px-4 py-2 text-gray-900">&#8377; {totals.totalCost}</td>
                                  </tr>
                                </>
                                  );
                            })()
                    ...
                    <tr className="hover:bg-amber-100 transition-all duration-300">
                        ...
                        <td className="px-4 py-2 text-gray-900">{order_details.payment_type ? "UPI / cards" : "cash"}</td>
                    </tr>
                    </tbody>
                </table>
